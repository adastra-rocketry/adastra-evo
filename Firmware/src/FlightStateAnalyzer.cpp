...
{
  if (DEBUG)
    Serial.println("BEGIN FlightStateAnalyzer::Loop()");
  ...
  else if (State.VehicleState == VehicleStateType::Ascending && State.CurrentDataPoint.Altitude < State.HeighestAltitude - 2)
  {
    State.UpdateFlightState(VehicleStateType::Descending);
  }
  else if (State.VehicleState == VehicleStateType::Descending && State.CurrentDataPoint.Altitude < State.Settings.LaunchAltitude + 5)
  {
    State.UpdateFlightState(VehicleStateType::Landed);
  }
  if (DEBUG)
    Serial.println("END FlightStateAnalyzer::Loop()");
}